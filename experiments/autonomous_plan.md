...
### 05:00 — Phase 4 深度分析 + Phase 5 收尾
- **問題段逗號 rank 分佈**：rank=2 佔 9.1%，rank<=5 佔 29.5%，rank<=10 佔 42.4%。逗號從未排第 1
- **logit gap 非常小**：top-20 近失時刻的 gap 僅 0.02-0.80 logits。最接近的（當然→如果）gap=0.05，幾乎是擲硬幣
- **常見前文 token**：「的」13次、「說」9次、「就是」7次、「然後」6次——都是自然的分句界限詞
- **正常段逗號間距**：median=11 字、mean=13.1 字、P25=7、P75=17
- **段長無差異**：問題段 mean=50 字、正常段 mean=54 字——長度不是問題
- **Phase 1-3 已 commit (018ba24)**
- 所有實驗任務完成，標記收尾
