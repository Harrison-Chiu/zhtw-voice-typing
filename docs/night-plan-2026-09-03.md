...
## 11. 早晨摘要（2026-09-03 夜間，執行者留）

### 做完什麼

T1–T4 全部完成，T5 的加分項除 T2.5 外也都做完。commit 都在本機 `main`，**沒有 push**。

| 任務 | 結果 |
|------|------|
| T1–T4 | 完成（詳見「4. 進度紀錄」） |
| T5.1 | 使用者裁決取消 |
| T5.2 | roadmap 狀態改成「規格／執行」兩軸 + 前置欄 `54e1a61` |
| T5.3 | `docs/index-html-gaps.md` 落差盤點（只盤點，翻新屬 F 線）`4a56599` |
| T5.4 | 審核佇列多來源預標註排序 `ece7389` |
| T5.5 | temperature fallback 決定性擴大重測 `b591c7c` |
| T5.6 | benchmark 提案與實作位置對齊 `19f6f2b` |
| T2.5 | **未做**，阻塞，見第 8 節 |

兩個影響後續判斷的結果：

1. **固定 `temperature=0` 不是 production 改善提案**（`a81b7d0` 已寫進 CLAUDE.md
   已確立決策）。100 段隨機分層樣本上 pinned 拿到 100/100 逐字一致，但不一致的那 3 段
   內容全是幻覺，pinned 只是讓幻覺變穩定；而且它讓 12 段的轉錄時間掉到 1.5 秒門檻以下，
   其中 2 段正是 production 靠 `hallucination-rejected` 擋掉的——等於把現行偵測訊號拿掉。
2. **審核佇列現在有排序**。三來源分歧最高的 9 段最該先聽。但量尺會摺疊標點，
   所以 `no-comma-long` 那 40 段的低分歧**不代表**該訊號誤報，別照那個數字砍訊號。

### 卡在哪

- **人工審核**仍是 A 與 E 兩線共同、也是擋住最多下游工作的單一瓶頸。機器能做的排序
  已經做完了，剩下的需要人耳。
- **T2.5** 需要使用者指定服務／金鑰，並同意把個人逐字稿送往該服務（不可逆的對外行為，
  夜間一律不做）。

### 第一件建議接手的事

開 `data/logs/review/queue.html`，照 `data/logs/review/preannotation_*.json` 的 `rank`
順序聽**分歧最高的那 9 段**。那是投入產出比最高的一步：聽完就同時解開 E 線訊號的
precision／recall 與 A 線 gold 的第一批樣本，而下游的 B 線基準、codec 實驗與凍結回歸集
都在等這個。

聽之前值得先看 `git log --oneline` 決定要不要 push；夜間所有 commit 都還在本機。
