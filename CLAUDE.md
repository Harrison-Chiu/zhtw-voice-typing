...
- **麥克風可用性看「有沒有資料流」，不看音量（2026-09-19）**：判定擷取故障的唯一訊號是
  ...
  `no_data_error_sec` 3.0 停止並保留），與輪詢頻率脫鉤。
  **警示音的標準不是「裝置壞了沒」，而是「這次錄音是不是白費、使用者現在還來得及補救」**
  ——涵蓋兩種情況（2026-09-19 實測補上第二種）：① 資料流中斷；
  ② 裝置正常但整段錄音從第一個 sample 起就一直近零（麥克風被關掉或靜音最常見）。
  第二種原本只有工作列符號、沒有聲音：實測關掉麥克風錄了 9.4 秒與 14.7 秒，
  log 顯示 `0 個語音片段`、watchdog 完全沒觸發，使用者要盯著工作列才會發現。
  判斷條件是「這次錄音從未有過高於門檻的音量」，所以講到一半停頓不會響；
  每次錄音最多響一次，可用 `microphone.alert_on_no_speech` 關閉。
