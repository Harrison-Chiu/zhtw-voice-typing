...
- **Fun-ASR-Nano 評測結論（暫不採用·版本卡關，2026-06-25）**：隔離環境 `.venv-funasr`（uv venv py3.11）裝 funasr 一次成功——**py3.11 的 resolver 自動避開了 conda py3.13 踩到的 llvmlite 0.36 地獄**（這驗證了延後到隔離環境的判斷對）。但 `pip install funasr`（1.3.14）載入 `FunAudioLLM/Fun-ASR-Nano-2512` 時缺一整批 `ctc_decoder.*` / `ctc.ctc_lo.*` 權重，輸出退化成單字+逗號重複數百次的垃圾（native 與 hotword 皆然）。研判 funasr 版本與此 2025/12 模型不相容。**重試方向**：依 FunAudioLLM 官方 GitHub 的 requirements 裝指定 funasr 版本（HF 模型卡只寫 `pip install -r requirements.txt`，版本在 GitHub repo）。腳本 `scripts/test_funasr_nano.py`（隔離環境跑）
